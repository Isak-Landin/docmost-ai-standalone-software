...
- Fixing responsive shell, header, menu, or dashboard width/containment issues
- Hardening customer-visible UI fixes so restorative changes do not introduce new regressions
...
#### Responsive shell/header containment rule

When the issue is mobile overlap, clipped header content, or dropdown/menu width drift:

1. Treat it as a `web-coder` issue class first, then pair with any repo-specific ownership skill.
2. Read `references/frontend-regression-guards.md` before widening the change.
3. Use `references/css-styling.md` for the exact containment pattern details.
4. Keep the change restorative - fix width and containment first, not typography or cosmetic styling.
...
This skill includes 16 reference files covering both general web development and regression-safe implementation:
...
16. [Frontend Regression Guards](references/frontend-regression-guards.md) - restorative UI fix rules that protect dynamic data, containment, and existing DOM hooks
