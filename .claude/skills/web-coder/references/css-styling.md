...
### Responsive shell containment pattern

For shell/header/menu/mobile overflow issues, use containment rules before redesigning the component:
...
Good pattern:
- use `minmax(0, 1fr)` and `min-width: 0` so the content column is actually allowed to shrink
- cap action/button/dropdown widths with `width: min(...)` or `max-width`
- ellipsize one-line labels instead of letting them push outside the viewport

Bad pattern:
- unconstrained mobile header/menu/button widths
- flex/grid children that never get `min-width: 0`
- letting username + hint text decide the shell width on narrow screens

When this pattern is applied to a live customer/admin surface, pair it with
`references/frontend-regression-guards.md` so containment fixes do not break dynamic
states, markup hooks, or repo ownership boundaries.
