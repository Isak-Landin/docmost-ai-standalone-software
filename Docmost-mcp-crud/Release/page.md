...
- Write-path ingest escaping that mirrors the renderer: every write escapes flanking `_`/`__` and non-schema HTML tags outside code, so code, paths like `app/__init__.py`, and stray tags reach Docmost as literal text instead of being parsed as bold or silently dropped. Two edges remain (Docmost's `$`/math grab on ingest and a deferred two-space ordered-indent input case) - see Known Limitations.
