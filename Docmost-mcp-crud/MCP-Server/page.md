...
| `create_local_replica_page` | `space_id: UUID`, `title`, `content?`, `parent_page_id?`, `parent_local_path?`, `local_root?: str` | Scaffold a local-only page in the selected local working copy before any remote page exists |
| `get_sync_status` | `space_id: UUID`, `include_synced?: bool`, `local_root?: str` | Get sync status for a space in the selected working copy |
| `get_sync_diff` | `space_id: UUID`, `page_id?: UUID`, `local_path?: str`, `include_synced?: bool`, `local_root?: str` | Get line-based local-vs-remote diff hunks |
| `pull_replica` | `space_id: UUID`, `page_ids?: UUID[]`, `local_paths?: str[]`, `force?: bool`, `local_root?: str` | Pull remote Docmost content into the selected local working copy |
| `push_replica` | `space_id: UUID`, `page_ids?: UUID[]`, `local_paths?: str[]`, `force?: bool`, `local_root?: str` | Push selected local working-copy changes back to remote Docmost |
...
- Use `create_local_replica_page(..., local_root?)` for local-first page creation in the selected working copy
