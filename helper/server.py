...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "resync_space(space_id) is an occasional whole-space variant: it re-renders EVERY page from "
        "Docmost on the server (even unchanged ones) then runs the same two-way reconcile. Use it "
        "when every page must be brought into sync regardless of whether it changed - e.g. after a "
        "server-side rendering change so pages never re-edited still pick up the corrected "
        "rendering. It heals via pull, surfaces conflicts the same way, never force-pushes, and "
        "adds reanchored_count to the summary. Prefer plain sync_space for everyday work.\n\n"
